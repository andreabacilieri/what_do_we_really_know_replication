###############################################################################
# This script produces Figure E.2: Mean in- and out-degree by industry 
...
# OUTPUT: 
# to results/figures
#     - E2_fig_average_in_out_degree_industry.png
...
df_allecon_h <- df_h[, .(mean_deg = mean(deg, na.rm = T),
                         mean_outD = mean(outD, na.rm = T),
                         mean_inD = mean(inD, na.rm = T)),
                     by = year]
# delete missing values (indu_codes)
df_h <- na.omit(df_h)
cols <- grep("^indu_code_", names(df_h), value = TRUE)
df_h <- melt(df_h,
             id.vars = c("year", "deg", "outD", "inD"),
             measure.vars = cols,
             value.name = "indu_code",
             variable.name = "code_level")
df_h <- setDT(df_h)

#df_h <- melt(df_h, id.vars = c("year", "deg", "outD", "inD"), 
 #            measure.vars = patterns("^indu_code_"), 
  #           value.name = "indu_code", 
   #          variable.name = "code_level")
# Get average deg
df_h <- df_h[, .(mean_deg = mean(deg, na.rm = TRUE),
                 mean_outD = mean(outD, na.rm = TRUE),
                 mean_inD = mean(inD, na.rm = TRUE)), 
             by = .(year, code_level, indu_code)]


## ECUADOR ##
# Get average degree for the whole economy
df_allecon_e <- df_e[, .(mean_deg = mean(deg, na.rm = T),
                         mean_outD = mean(outD, na.rm = T),
                         mean_inD = mean(inD, na.rm = T)),
                     by = year]
...
df_e <- melt(df_e,
             ...
             variable.name = "code_level")
df_e <- setDT(df_e)
#df_e <- melt(df_e, id.vars = c("year", "deg", "outD", "inD"), 
 #            measure.vars = patterns("^indu_code_"), 
  #           value.name = "indu_code", 
   #          variable.name = "code_level")
# Get average deg
df_e <- df_e[, .(mean_deg = mean(deg, na.rm = TRUE),
                 mean_outD = mean(outD, na.rm = TRUE),
                 mean_inD = mean(inD, na.rm = TRUE)), 
             by = .(year, code_level, indu_code)]
...
# ------------------------------------------------------------------------------
#  Plot: Mean in-degree vs mean out-degree by industry
# ------------------------------------------------------------------------------
df_plot_e <- df_e[code_level == 'indu_code_1' & year == 2015]
df_plot_h <- df_h[code_level == 'indu_code_1' & year == 2021]

ymin <- min(df_plot_e$mean_outD)
ymax <- max(df_plot_e$mean_outD)

p1 <- ggplot(df_plot_e, 
             aes(x = mean_inD, y = mean_outD, label = indu_code)) +
...
p2 <- ggplot(df_plot_h, 
  ...
  annotate("text", x = 10, y = 200, label = "Hungary, 2021", 
           color = "black", hjust = "left", size = 3) +
...
# Combine plots
g <- ggarrange(p1, p2 + rremove('ylab') + rremove("y.text") + rremove("y.ticks"), 
               nrow = 1, ncol = 2, 
               common.legend = F,
               widths = c(1.1, 1), font.label = element_text(size = rel(1)))
...
filename <- paste(dirOutput, '/E2_fig_average_in_out_degree_industry.png', sep = '')
ggsave(filename, g, width = width_l, height = height_l, dpi = 600)

print('Figure E.2: Mean in- and out-degree by industry exported.')
